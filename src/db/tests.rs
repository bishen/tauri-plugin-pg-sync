...
mod local_db_tests {
    ...
    fn test_ensure_table_creates_sync_fields() {
        ...
        db.ensure_table("users", &columns).expect("Failed to create table");
    ...
    fn test_insert_and_find_by_id() {
        ...
        let id = db.insert("items", &data, hlc, &node_id).expect("Insert failed");
    ...
    fn test_update_increments_version() {
        ...
        let id = db.insert("items", &json!({"title": "Original"}), "1:0:n", &node_id).unwrap();
        ...
        // 更新
        db.update("items", &id, &json!({"title": "Updated"}), "2:0:n").unwrap();
    ...
    fn test_soft_delete() {
        ...
        let id = db.insert("items", &json!({"title": "To Delete"}), "1:0:n", &node_id).unwrap();
    ...
    fn test_changelog_recording() {
        ...
        // 记录变更
        db.record_change("items", "id-1", "INSERT", "1:0:n", Some(r#"{"title":"test"}"#)).unwrap();
        db.record_change("items", "id-1", "UPDATE", "2:0:n", Some(r#"{"title":"updated"}"#)).unwrap();
    ...
    fn test_query_with_conditions() {
        ...
        db.insert("users", &json!({"name": "Alice", "age": 25}), "1:0:n", &node_id).unwrap();
        db.insert("users", &json!({"name": "Bob", "age": 30}), "2:0:n", &node_id).unwrap();
        db.insert("users", &json!({"name": "Charlie", "age": 25}), "3:0:n", &node_id).unwrap();
    ...
    fn test_advanced_query_options() {
        ...
        db.ensure_table("products", &[
            ("name".to_string(), "TEXT".to_string()),
            ("price".to_string(), "REAL".to_string()),
        ]).unwrap();
        
        db.insert("products", &json!({"name": "Apple", "price": 1.5}), "1:0:n", &node_id).unwrap();
        db.insert("products", &json!({"name": "Banana", "price": 0.8}), "2:0:n", &node_id).unwrap();
        db.insert("products", &json!({"name": "Cherry", "price": 3.0}), "3:0:n", &node_id).unwrap();
    ...
    fn test_count() {
        ...
        db.insert("items", &json!({"category": "A"}), "1:0:n", &node_id).unwrap();
        db.insert("items", &json!({"category": "A"}), "2:0:n", &node_id).unwrap();
        db.insert("items", &json!({"category": "B"}), "3:0:n", &node_id).unwrap();
        
        assert_eq!(db.count("items", None).unwrap(), 3);
        assert_eq!(db.count("items", Some(&json!({"category": "A"}))).unwrap(), 2);
    }
    ...
    fn test_table_exists() {
        ...
        db.ensure_table("test_table", &[("col".to_string(), "TEXT".to_string())]).unwrap();
    ...
    fn test_list_tables() {
        ...
        db.ensure_table("table_a", &[("col".to_string(), "TEXT".to_string())]).unwrap();
        db.ensure_table("table_b", &[("col".to_string(), "TEXT".to_string())]).unwrap();
